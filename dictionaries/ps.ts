...
//
// 🛠️ به‌روزرسانی (بخش «یکجا چیست؟»): معادل پشتوی همان به‌روزرسانی fa.ts.
//

// مسیر فایل: dictionaries/ps.ts
//
// 🛠️ به‌روزرسانی (فاز ۱۰ موبایل — قابلیت «ولایت»): معادل پشتوی همان به‌روزرسانی fa.ts — بخش
// province (هم‌محتوا با dict.province وب: src/dictionaries/ps.ts) و چهار کلید تازه‌ی
// invalidProvince اضافه شدند.
//
...
//
// اصلاح ممیزی تسک ۹ فاز M00B: کلید common.comingSoon تازه اضافه شد — قبلاً متن معادل فارسی‌اش
// مستقیم داخل components/PlaceholderScreen.tsx هاردکد بود (نقض الزام قطعی ۲)، پس اصلاً معادل
// پشتویی نداشت. ترجمه‌ی زیر هم مثل بقیه‌ی این فایل، کار هوش مصنوعی است، نه مترجم بومی‌گوی پشتو —
// پیش از تحویل نهایی بهتر است یک‌بار توسط یک فارسی/پشتو-زبان بررسی شود.
//
// 🛠️ به‌روزرسانی فاز M02 (تسک ۷ — صفحه‌ی «آگهی‌های من»): کلید marketplace.index.myListingsButton
// و بخش کامل marketplace.myListings تازه اضافه شدند، هم‌زمان و هم‌محتوا با همین افزوده در
// dictionaries/fa.ts (نقشه‌راه از قبل متنی برای این صفحه ننوشته بود). این ترجمه‌ی پشتو هم، مثل
// بقیه‌ی این فایل، کار هوش مصنوعی است نه مترجم بومی‌گوی پشتو — پیش از تحویل نهایی بهتر است بررسی
// شود؛ واژگان («اعلان»، «لړلیک»، «تایید»، «ړنګ») عمداً از باقی همین فایل (بخش‌های
// marketplace.index/detail بالاتر) عیناً وام گرفته شدند تا یکدست بمانند.
//
// 🛠️ به‌روزرسانی فاز M03، تسک ۷ (جستجوی دستی شهر/منطقه به‌جای GPS، ماژول حمل‌ونقل): همان پنج
// کلید تازه‌ی transport.list که در dictionaries/fa.ts اضافه شدند (searchPlaceholder،
// searchButton، searchingButton، searchNotFoundNotice، sortedBySearchNotice) + به‌روزرسانی
// locationDeniedNotice — عیناً و هم‌محتوا اینجا هم اضافه شدند؛ دلیل فنی کامل (چرا این ماژول با
// marketplace/services/realEstate فرق دارد) در کامنت بالای fa.ts مستند شده. این ترجمه‌ی پشتو هم،
// مثل بقیه‌ی این فایل، کار هوش مصنوعی است نه مترجم بومی‌گوی پشتو — پیش از تحویل نهایی بهتر است
// بررسی شود.
//
// 🛠️ به‌روزرسانی فاز M04، تسک ۱ (فهرست/جستجوی متخصصین): دو کلید تازه به services.list اضافه شد
// (locationBlockedNotice، locationServicesDisabledNotice)، عیناً هم‌محتوا با dictionaries/fa.ts؛
// دلیل فنی کامل (چرا این تسک از lib/location.ts مشترک استفاده کرد) در کامنت بالای fa.ts مستند
// شده. این ترجمه‌ی پشتو هم، مثل بقیه‌ی این فایل، کار هوش مصنوعی است نه مترجم بومی‌گوی پشتو — پیش
// از تحویل نهایی بهتر است بررسی شود.
export default {
  "meta": {
